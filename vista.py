...
class VistaSesion:
    def __init__(self):
        ...
        # Encabezado superior verde con texto y emoji de hoja
        header = tk.Frame(self.ventana, bg="#7AC74F", height=50)
        header.pack(fill="x")
        tk.Label(header, text="🌿", bg="#7AC74F", fg="white", font=("Arial", 20)).pack(side="left", padx=10, pady=5)
        tk.Label(header, text="Vivero Vital", bg="#7AC74F", fg="white", font=("Arial", 20, "bold")).pack(side="left", pady=5)

        # Contenedor central beige claro con borde redondeado
        contenedor = tk.Frame(self.ventana, bg="#FFFDEB", bd=2, relief="solid")
        contenedor.place(relx=0.5, rely=0.5, anchor="center")

        # Añadir padding para simular borde redondeado (Tkinter no soporta borde redondeado nativamente)
        contenedor.config(padx=40, pady=30)

        # Título en color naranja claro
        tk.Label(contenedor, text="Iniciar Sesión", bg="#FFFDEB", fg="#E5B94E",
                 font=("Arial", 22, "bold")).pack(pady=(0, 20))

        # Campos de texto con fondo crema y borde sutil, centrados
        tk.Label(contenedor, text="Usuario:", bg="#FFFDEB", font=("Arial", 12)).pack(anchor="w")
        self.entryUsuario = tk.Entry(contenedor, width=30, bg="#FAF8E1", relief="solid", bd=0.5, fg="#4A4A4A")
        self.entryUsuario.pack(pady=5)

        tk.Label(contenedor, text="Contraseña:", bg="#FFFDEB", font=("Arial", 12)).pack(anchor="w")
        self.entryContrasena = tk.Entry(contenedor, width=30, show="*", bg="#FAF8E1", relief="solid", bd=0.5, fg="#4A4A4A")
        self.entryContrasena.pack(pady=5)

        # Botón Confirmar verde, texto blanco, sin borde
        self.btnConfirmar = tk.Button(contenedor, text="Confirmar", bg="#7AC74F", fg="white",
                                      font=("Arial", 12, "bold"), width=15, relief="flat", command=None)
        self.btnConfirmar.pack(pady=15)

        self.lblMensaje = tk.Label(contenedor, text="", bg="#FFFDEB", fg="red", font=("Arial", 11))
        self.lblMensaje.pack(pady=5)

        # Barra inferior amarilla
        tk.Label(self.ventana, bg="#FFD166", height=2).pack(side="bottom", fill="x")
